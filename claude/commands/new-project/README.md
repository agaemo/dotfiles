...
| `refactorer` | 動くコードの構造整理 | 振る舞いを変えずに内部構造を改善。新機能追加は行わない。 |
| `architect` | 設計の大規模見直し | アーキテクチャ評価・テックデット特定・大規模リファクタ方針。 |
| `sre` | パフォーマンス・インフラ懸念 | インフラ・パフォーマンス・Web 表示速度のレビュー。 |
| `scorer` | コードベースの健全性確認 | コードベースの健全性を 6 観点で定期評価。実装は行わない。 |
| `release-planner` | 本番リリース前 | リリース戦略・デプロイ計画・ロールバック手順の策定。 |
