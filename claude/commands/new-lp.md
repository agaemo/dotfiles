...
--- 住所・電話番号・SNSなどの連絡先データ ---

サンプルLP（ユーザーから実際の情報の指定がない場合）では、以下のルールで仮データを使うこと:

  住所:
    REQUIRED: 郵便番号は「〒000-0000」など、実在しないと一目でわかる形式にする
    REQUIRED: 都市名・区・町名はすべて架空の名前にする（例: 架空都サンプル区見本町）
    PROHIBITED: 実在する郵便番号・都道府県・市区町村・番地を組み合わせること

  電話番号:
    REQUIRED: 市外局番を「000」、残りも「0000-0000」など、実在しないと明らかな形式にする
    PROHIBITED: 実在しうる市外局番（03-, 06-, 090-など）を使うこと

  SNS・URL:
    REQUIRED: href は「#」か「javascript:void(0)」にする
    PROHIBITED: https://www.instagram.com/ など実在サービスのルートURLをリンク先に使うこと
    理由: クリックで意図しない第三者サービスに遷移させてしまうため

  アクセス案内（最寄り駅など）:
    REQUIRED: 架空の路線名・駅名を使う（例: 架空線「サンプル駅」北口より徒歩5分）
    PROHIBITED: 実在する駅名・バス停名を使うこと
