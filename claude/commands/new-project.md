...
STEP 6: 実装
  ...
  実装は「機能フェーズ単位」で進め、各フェーズ完了後に必ず動作確認を行うこと。
  一気に全フェーズを実装すると不具合の原因特定が困難になり、手戻りコストが増大する。

  推奨サイクル（1フェーズごとに繰り返す）:
    1. そのフェーズのコードを実装する
    2. `pnpm dev`（または該当のdevコマンド）でサーバーを起動
    3. docs/stories.md の当該フェーズの受け入れ条件を実機で確認する
    4. エラーがあれば即修正してから次のフェーズへ進む

  HAS_FRONTEND == true の場合、以下の順序で実装・確認すること:
    □ Phase 1: DB・認証基盤 → ログイン/ログアウト確認
    □ Phase 2: 管理機能（マスタ・ユーザー等）→ CRUD確認、RBACの動作確認
    □ Phase 3: コア機能の登録・一覧・詳細 → 基本フロー確認
    □ Phase 4: 操作系（ステータス変更・コメント・担当者等）→ インタラクション確認
    □ Phase 5: ダッシュボード・集計 → 集計値確認

  NOTE: フェーズの分け方はプロジェクトによって異なるが、
        「依存するデータが揃っていない状態で先行する機能を実装しない」原則は共通。
