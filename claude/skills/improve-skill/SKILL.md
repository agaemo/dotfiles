...
FILE_ARGS: 引数からスコア閾値テキスト（"点"・"以上"・"超える"・"score"・"まで"・"目標" 等を含むトークン）を除いた部分
...
ELIF FILE_ARGS が複数パス（スペース区切りで2つ以上のファイルパス）:
  ...
  → MULTI_FILE_PROCESS を実行する
ELSE:
  READ: <対象スキルファイル>  ← FILE_ARGS のパス or ASK回答のパス
  IF ファイルが存在しない:
...
MULTI_FILE_PROCESS:
  ...
  全ファイル完了後、ステップ2.5を一度だけ実行してからステップ5に進む
