...
IF 引数が "all":
  ...
  PROCESS: 以降のステップ2・2.5・3・4を全ファイルに対して順に実行する。
           全ファイルの処理が完了したら、ステップ5に進む。
...
ELIF 引数が複数パス（スペース区切りで2つ以上のファイルパス）:
  ...
  PROCESS: "all" と同様に全ファイルに対して順に実行し、ステップ5で一括提示する
ELSE:
...
```
IF 2/5 以下の次元が 3つ以上（N/A の次元は除外して数える）:
  WARN: 差分修正より全書き直しが適切な可能性があります
  ASK USER:
    「複数の次元でスコアが低く（2/5 以下が 3つ以上）、差分修正より全書き直しを推奨します。」
