...
verbosity = 簡潔 の場合: ファイル保存は行わない。`{evolution_result}` の「Wardley Map（テキスト表現）」セクションと `{strategy_result}` の「優先度の高いアクション（TOP 3）」セクションを会話上に出力する。`{evolution_result}` に「（推定）」が含まれる場合は該当コンポーネント名を列挙し「`{コンポーネント名}` は確信度の低い推定です。気になる場合は『○○をもっと詳しく』と聞くと深掘りできます」と一言添えてから、ステップ5（対話）に進む。
...
`{evolution_result}` に「（推定）」が含まれる場合: 該当コンポーネント名を列挙し「`{コンポーネント名}` は確信度の低い推定です。気になる場合は『○○をもっと詳しく』と聞くと深掘りできます」と一言添える。
