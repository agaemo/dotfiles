...
IMPORTANT: 会話・保存ファイルのいずれでも分析結果を最初に提示する際は、次の順序で提示すること: 0)「アイデア生成とは」を一言 → 1) 全体マップ（mindmap）の図 → 2)「用語集」。保存ファイルでは2)の「用語集」を `<details><summary>用語集</summary>` 〜 `</details>` で囲み折りたたみ表示にする（会話上では折りたたみは効かないため通常の箇条書きのままでよい）。
