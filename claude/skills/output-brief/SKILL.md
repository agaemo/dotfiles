...
```
SKILL_DIR = このスキルを呼び出したときに Read した SKILL.md の絶対パスから /SKILL.md を取り除いた文字列
例: /Users/xxx/.claude/skills/output-brief
```
...
指示から以下3点を把握する。**全て読み取れる場合はステップ2へ直行する。不明な点があれば1回でまとめて質問する:**
...
次に `AskUserQuestion` でレイアウトを選んでもらう:
...
次に `AskUserQuestion` でカラーテーマを選んでもらう:
...
全 24 種のサンプルは `$SKILL_DIR/assets/samples/<layout>-<color>.html` に保存されている。
...
修正を求められた場合は、変更したい項目（レイアウト / カラー / 形式 / 保存先）を確認してから該当するステップに戻って再提示する（最大3ラウンド。収束しない場合は中断してユーザーに方向性を確認する）。
...
- ファイル保存の場合:
  1. 指定パスに既存ファイルがあれば、上書きするか別名にするかをユーザーに確認する
     WAIT_FOR: ユーザーの選択
