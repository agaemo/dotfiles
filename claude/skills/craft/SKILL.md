...
中核的な依存関係（ビルドツール・devサーバー等）も対象。** scaffold直後に主要な依存関係の
バージョンを確認し、1.0未満のベータ版・実験的リリースが含まれる場合は、その旨と既知の制約を
ユーザーに伝えてから先に進む（気づかず使い続けない）。
...
原因になりうる。また、生成コマンドの引数仕様（カレントディレクトリを指定できるか、force相当の
挙動、既存ファイルがある場合の扱い等）は言語・ツール・バージョンによって異なるため、本番の
対象ディレクトリに影響が出る前に、一時ディレクトリ等で軽く試してから本実行すること。

IMPORTANT: **コンテナ（Docker等）で環境構築する場合、プロジェクト生成に使うツールチェーン
（一時コンテナ等）のバージョンと、実際にアプリを実行する環境（本番相当コンテナ）のバージョンを
一致させること。** 言語ランタイムのバージョンが生成時と実行時で異なると、依存関係のロック
ファイルに実行環境と互換性のないバージョン制約が書き込まれ、後になって競合が発覚する
