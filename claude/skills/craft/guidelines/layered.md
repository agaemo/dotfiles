...
> planner エージェントがアーキテクチャ選定時に参照するガイドライン。
...
IMPORTANT: ここでの命名は一例（TypeScript想定）。実装言語の予約語と衝突する場合は
自然な代替名にすること（例: Rustの`impl`、Pythonの`class`等）。
ディレクトリ名を決める前に、選定言語の予約語一覧を確認すること。
