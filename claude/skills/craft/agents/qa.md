...
4. 受け入れ条件に対応するテストが存在するかを評価する。ファイル名の一致だけで判定せず、Grep で `describe(` / `it(` / `test(` のテスト名を横断検索し、該当ファイルを Read してテスト内容（アサーション対象）が受け入れ条件に対応しているかを確認する。1テストが受け入れ条件の一部のみ対応する場合は「テスト存在」列に「一部対応」と記載する
   - 受け入れ条件が「別プロセス・別セッション・外部サービス・実際のユーザー操作など、
     このセッションから直接は発火できない引き金」を前提とする場合、既存テスト（`curl`相当の
     シミュレーションを含む）は「受信側の実装の検証」であって「本番の引き金からの検証」ではない。
     この区別を「備考」列に明記し、本番の引き金での確認を不足テストケース（必須）として提案する
     （`flows/build/SKILL.md`の「検証の真正性」と同じ基準で判定する）
5. 不足テストケースを提案する
