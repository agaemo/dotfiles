...
ASK USER:
  1. 対象テーブル・カラムは何か
  2. 変更種別は何か（追加・削除・型変更・インデックス追加等）
  3. 既存データの有無（バックフィルが必要か）
WAIT_FOR: ユーザーの回答

IF consult から SUMMARY が渡されている かつ 上記が SUMMARY に含まれている:
  再確認せず SUMMARY の内容を使う
```
...
```
GATE: 本番反映の実行内容（対象テーブル・推定実行時間・メンテナンス時間帯）をユーザーに提示し、承認を得ること
WAIT_FOR: ユーザーの承認
PROHIBITED: 承認前に本番DDLを実行すること
```
