...
PLAN:
  ...
  IF STACK が static 以外（コードの実装・修正を伴う）:
    # new-project の planner が行うレビュートラック宣言を、consult 経由でも必ず行う。
    # 宣言なしで build フローに渡すと、規模に関わらず自動的にフル（トラックC）が適用される
    ANALYZE: 変更規模・リスクからレビュートラックを提案する（上から順に該当するものを採用する。
      複数該当する場合は下にある基準ほど優先する）
      - 変更ファイル数が5件以下 かつ DB・認証・決済等の重要ロジックを含まない
        （学習用サンプル・PoC・使い捨てプロジェクト等） → A（軽量: verify → code-reviewer）
      - 変更ファイル数が6件以上、または通常規模の機能追加・改修 → B（標準: verify → qa → code-reviewer）
      - 認証・決済・個人情報・外部公開APIなど重大な変更を含む → C（フル。ファイル数によらず優先）
    PRESENT: 提案したトラックと選定理由を1文で示し、変更希望があるか確認する
    WAIT_FOR: ユーザーの承認（そのまま/別トラック希望どちらも受け付ける）
    計画フォーマットの冒頭に確定したトラックを明記する（下記参照）

  計画フォーマット：
    ## レビュートラック: [A / B / C]（static の場合はこの行を省略）
...
IF .craft/docs/ ディレクトリが存在する:
  FIX_FILE = .craft/docs/{YYYYMMDD}-{slug}.md
    ...
    # 同じ機能追加で複数回改修する場合も、都度新規のFIX_FILEを作成する（既存への追記はしない）

  # 相談対象が既存システムで、かつ今回の変更がDBスキーマ・API・画面遷移・認証設計のいずれかを
  # 新規に伴う場合のみ「## 基本設計」を書く。該当しない項目は含めない（表を空欄で出力しない）。
  # new-project 相当の重量テンプレートを consult 側に複製せず、必要な項目だけ都度参照する
  # 「新規に伴う」の判定基準（狭義）: 新規テーブル/カラムの追加、新規APIエンドポイントの追加、
  # 新規画面の追加、認証方式の新規導入・変更を指す。既存エンドポイント・既存テーブルの内部処理
  # （バリデーション強化・ロジック修正等、インターフェース自体は変わらないもの）は含まない
  IF 相談対象 == "既存システム" AND 今回の変更がDBスキーマ・API・画面遷移・認証設計のいずれかを新規に伴う:
    READ {SKILL_DIR}/flows/new-project/templates.md の「テンプレート: 基本設計書」セクション
    EXTRACT: 該当する項目（例: DBスキーマ設計・API設計・認証設計・画面遷移図）の見出し・表形式
      のみを流用する。デザインシステム概要・ADR等、今回の変更に関係しない項目は含めない
      NOTE: 値は plan.md の「基本設計」節を経由せず（consult の plan.md は通常この節を持たない）、
        今回の調査結果・PLAN で確定した実行計画の内容から直接埋める
    SET 基本設計セクション = 上記で抽出した内容（"## 基本設計" 見出し配下）
  ELSE:
    SET 基本設計セクション = なし

  WRITE {FIX_FILE}: 以下の内容を記録する
    - ## 課題: 今回対応した問題の概要
    - {基本設計セクション}（あれば「## 課題」の直後、「## 実行計画」の前に挿入）
    - ## 実行計画: フェーズ一覧と変更ファイル
