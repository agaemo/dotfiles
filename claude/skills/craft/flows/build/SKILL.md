...
IF NOT EXISTS(README.md):
  ...
  INCLUDE:
    ...
    - セットアップ手順（git clone 〜 依存インストール 〜 .env 設定（あれば）。一度だけ行う導入作業）
    - コマンド一覧（dev / build / test 等）
    - IF STACK == "node": 環境変数（キー名と説明のみ。実際の値は書かない）
    - IF HAS_FRONTEND == true、またはCLIツール等の実行体がある場合:
      「使い方」をセットアップ手順とは独立した節にする。
      IMPORTANT: セットアップ手順（一度だけの導入作業）と混同しないこと。
      内容は「できあがったものを実際にどう使うか」:
        - デスクトップアプリ: 起動後の見た目・常駐の仕方・主要な操作・終了方法
        - CLIツール: 主要なサブコマンドと使用例
        - Webアプリ: 主要画面への到達方法・ログイン要否
        - 外部イベント/Webhookで動く実装（本アプリ自身がHooks等の受け口を持つ場合）:
          何がどう起きると反応するのか、本番の引き金を実際に発生させて確認する手順
          （上記「検証の真正性」で未検証だった場合は特に重要）
...
--- 検証の真正性（完了報告の前に必ず自問する） ---

「この機能が実際に使われるとき、引き金を引くのは誰か・何か？」を自問する。

- 答えが「このセッション自身（ツール呼び出しで直接発火できる）」→ 通常の動作確認でよい
- 答えが「別プロセス・別セッション・外部サービス・実際のユーザー操作など、
  このセッションから直接は発火できないもの」→ curl・テスト用ペイロード送信・
  モック呼び出し等での確認は「受信側の実装が正しいことの確認」にすぎない。
  「本番の引き金から本当に届くか」はまだ確認できていない
  （Webhook・Hook・cron・外部センサー・他ユーザーの操作等、個々のシナリオを
  列挙してルール化するのではなく、常にこの自問1つで判定すること）

IF 引き金が現在のセッションから直接発火できない:
  完了報告で以下を明確に分けて記載する（詳細は下記「完了報告」参照）:
    - 検証済み: [対象]の受信・処理ロジックは正しく動作する（検証方法: [curl等の手段]）
    - 未検証: 本番の引き金（[具体的に誰が・何をすれば発生するか]）からの動作
  IMPORTANT: 未検証の項目を「動作確認済み」「完了」と表現しないこと。
  ユーザー自身が本番の引き金を実際に発生させて確認できる具体的な手順
  （何をすればよいか・成功をどう見分けるか）を完了報告に含めること
...
IF HAS_FRONTEND == true:
  ENSURE: 開発サーバーが起動中であること
    IF 停止中: `make dev` をバックグラウンドで起動してから続ける
    IF 起動中: そのまま続ける
  NOTE: 「完了しました」と伝える時点でサーバーが止まっていると、ユーザーがブラウザを
    開いても確認できない。起動状態を必ず揃えてから報告すること。

  IF 認証機能を含む（plan.md に認証・ログイン・セッション等の記述がある）:
    RUN: `make seed` でテストユーザーを作成する
      IF `make seed` が存在しない:
        NOTE: curl・psql・seed スクリプト等、利用可能な手段でテストユーザーを1件作成する
    ASSERT: テストユーザーの作成に成功したこと（既存ユーザーとして出力された場合も許容）

REPORT TO USER:
  実装が完了しました。
  IF HAS_FRONTEND == true:
    - サーバー起動中: {plan.md の APP_PORT または 8080 等の実際のポート番号で URL を明示}
  ...
  NOTE: サーバー状態（起動中 or 停止中）を必ず明記すること。
    「動作確認できます」だけでは不十分。
  IF 上記「検証の真正性」で未検証の項目があった:
    「検証済み」と「未検証（本番の引き金からの動作）」を項目ごとに分けて明記し、
    未検証分の具体的な確認手順（誰が・何をすればよいか）を併記すること。
    IMPORTANT: 未検証の項目が残る状態を「完了しました」の一言だけで済ませないこと。

--- 完成物の解説ガイド（任意） ---

REPORT TO USER後、追加で確認する:
「プロジェクトの仕組みを説明するガイドを作りますか？」

IF ユーザーが希望した:
  インストールされているスキル一覧を確認し、ドキュメント・ガイド生成用途に
  使えるスキル（description に HTML/Markdown 生成・ブリーフィング等の記載があるもの）を探す。
  IF 該当するスキルが見つかった:
    「[スキル名]スキルでガイドを作成しますが、よいですか？」とユーザーに確認してから使う
  ELSE:
    Claude自身の判断で簡潔なMarkdown（概要・アーキテクチャ・使い方）を直接Writeする
