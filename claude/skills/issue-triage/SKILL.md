...
/issue-triage {issue番号1} {issue番号2} ...      # 複数issueを一括トリアージ（ステップ0-B参照）
...
1. 引数を解析する：
   - 数字トークンが2つ以上（スペース区切り）あれば `{issue番号リスト}` として扱い、**ステップ 0-B** へ進む
   - 数字トークンが1つだけなら `{issue番号}` として扱い、**ステップ 1** へ進む
...
### ステップ 0-B：複数issueの一括トリアージ（issue番号が2件以上指定された場合）

**解析フェーズ（並列・読み取り専用）**

各issueについて `fork`（Agent ツール, subagent_type: "fork"）を1issue1forkで起動し、ステップ1（issueを読む）〜ステップ3（方針の判定。ユーザーへの提示・承認は行わない）を実行させ、推奨方針（A/B/C）と提示内容を構造化して返させる。
IMPORTANT — PROHIBITED: この段階のforkに承認取得・git操作・ファイル変更を行わせること（読み取り専用の分析のみ）
WAIT_FOR: 全forkの結果

**承認フェーズ（親エージェントが一括提示）**

全issueの分析結果を一覧でまとめて表示し、issueごとに (y/N/wontfix) を確認する。
```
#42 【推奨】直接修正 — {概要}
#45 【要検討】計画が必要 — {概要}
#51 【wontfix推奨】 — {概要}

各issueについて承認してください（例: 42:y 45:N 51:wontfix）：
```
WAIT_FOR: ユーザーの回答（issueごとに y/N/wontfix を判定する）

**実行フェーズ（直接修正=Aのみ並列化）**

承認された issue のうち方針 A（直接修正）のものは、`fork`（Agent ツール, subagent_type: "fork", isolation: "worktree"）で1issue1forkに並列化し、ステップ4-Aの1〜3（オープンPR確認・ブランチ作成・修正適用）とビルド確認までを実行させ、diffを親に返させる。
IMPORTANT — PROHIBITED: この段階のforkにcommit・push・PR作成を行わせること（diff確認前のため）
WAIT_FOR: 全forkのdiff結果

親が全diffをまとめて提示し、最終承認を得る：
```
以下の変更をコミット・リモートにpushします：
#42: {diff の概要}
#51: {diff の概要}

コミット・pushするissueを選んでください（例: all / 42 / skip）：
```
WAIT_FOR: ユーザーの選択
PROHIBITED: 上記の最終承認を得ずにコミット・pushすること

承認されたissueについては、対応するforkに `SendMessage` でcommit・push・PR作成の続行を指示する（ステップ4-Aの6を実行させる）。承認されなかったissueのforkには worktree の破棄を指示する。
方針 B（計画が必要）が承認されたissueは並列化せず、issueごとに順番にステップ4-Bを実行する（複数PRにまたがる変更のため）。
方針 C（wontfix）が承認されたissueは、親エージェントがそのままステップ4-Cを実行する。

全issue完了後、「issue番号・方針・結果（PR番号 / close / スキップ）」の一覧を報告する。
...
- 複数issueの一括処理（ステップ0-B）でworktreeを使った場合、変更ありのworktreeは自動削除されない。全issue完了報告に各worktreeのパスを含め、PRマージ後にユーザー自身で削除してもらう
