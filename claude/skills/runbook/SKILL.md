...
- `/runbook` — 引数なしの場合はタスク名と補足情報を同時に確認する

---

## 手順

### ステップ1: タスクの特定と補足情報の収集

```
IF 引数でタスク名が指定されていない:
  ASK USER（1メッセージで同時に聞く）:
    「どのタスクのチェックリストを作成しますか？
    あわせて以下も教えてください（スキップ可）:
    1. 環境・対象: 本番 / ステージング / 特定サービス名など
    2. 技術スタック: 使用言語・FW・インフラなど（例: Next.js / PostgreSQL / AWS）
    3. 特に注意したい点: 過去の失敗・ハマりどころなど」
  WAIT_FOR: ユーザーの回答
  {task}    = 回答の先頭行またはタスク名として識別できる部分
  {context} = 回答の補足情報部分（補足なし・スキップ・省略の場合は ""）

ELSE（引数でタスク名が指定されている場合）:
  {task} = 引数として指定された値
  ASK USER:
    「{task} のチェックリストを生成します。以下を教えてください（スキップ可）:
    1. 環境・対象: 本番 / ステージング / 特定サービス名など
    2. 技術スタック: 使用言語・FW・インフラなど（例: Next.js / PostgreSQL / AWS）
    3. 特に注意したい点: 過去の失敗・ハマりどころなど」
  WAIT_FOR: ユーザーの回答
  IF 「スキップ」「省略」: {context} = ""
  ELSE: {context} = ユーザーの回答
ENDIF
```
...
```
CLASSIFY リスクレベル（{task} と {context} から判定）:
